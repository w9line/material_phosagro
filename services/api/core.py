...
def tool(name: str, args: dict[str, Any]) -> Any:
    ...
    if name == "get_inventory_summary":
        ...
        for item in groups:
            count = item["batch_count"] or 1
            item["average_raw_mass_kg"] = item["raw_mass_kg"] / count
            item["average_theoretical_active_mass_kg"] = item["theoretical_active_mass_kg"] / count
            item["average_available_active_mass_kg"] = item["available_active_mass_kg"] / count
        ...
        count = totals["batch_count"] or 1
        totals["average_raw_mass_kg"] = totals["raw_mass_kg"] / count
        totals["average_theoretical_active_mass_kg"] = totals["theoretical_active_mass_kg"] / count
        totals["average_available_active_mass_kg"] = totals["available_active_mass_kg"] / count
...
def route_intent(message: str, history: list[dict[str, str]] | None = None) -> dict[str, Any]:
    ...
    inventory_average_request = any(word in normalized for word in ("средн", "средняя", "в среднем")) and any(word in normalized for word in ("активн", "масса", "веществ"))
    inventory_summary_request = any(word in normalized for word in ("остат", "остал", "склад", "запас", "сырь", "че по", "потер")) and any(word in normalized for word in ("покажи", "покаж", "дай", "проверь", "посчитай", "сколько", "че по", "где", "какая", "какие"))
    if inventory_summary_request or inventory_average_request:
